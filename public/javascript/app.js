...
app.factory('dataFactory', ['$http', '$q', function($http, $q) {
  ...
  dataFactory.postData = function(user) {
    ...
    $http.get("data/" + user.loc.replace(/\s+/g, '-').replace("/", "-").toLowerCase() + "-" + user.age.replace(/\s+/g, '-').toLowerCase() + "-" + user.gen + ".json").
    success(function(data, status, headers, config) {
      deferred.resolve(data)
    }).
    error(function(data, status, headers, config) {
...
app.controller('myCtrl', ['$scope', 'dataFactory', function($scope, dataFactory) {
  ...
  $scope.Submit = function(){
    ...
    dataFactory.postData($scope.user).then(function(data) {
      ...
      $scope.showDist = !jQuery.isEmptyObject(data.district_crime)
      $scope.showYourGen = !jQuery.isEmptyObject(data.your_gen_crime)
      $scope.showYourAge = !jQuery.isEmptyObject(data.your_age_crime)
